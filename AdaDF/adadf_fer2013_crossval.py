...
class FER2013Dataset(Dataset):
    def __init__(self, data_path, transform=None):
        ...
        self.label = df['emotion'].values
        
    def __len__(self):
        return len(self.label)

    def __getitem__(self, idx):
        # Convert the space-separated string to numpy array
...
# Create FER2013 dataloaders
def get_fer2013_dataloaders(data_path, batch_size=64, num_workers=2, 
    ...
    full_dataset = FER2013Dataset(data_path, transform=data_transforms)

    # Create subsets using indices
    train_dataset = torch.utils.data.Subset(full_dataset, train_indices)
    
    # Create validation and test datasets with validation transforms
    full_dataset_val = FER2013Dataset(data_path, transform=data_transforms_val)
...
parser.add_argument('--num_samples', default=35887, type=int)  # Changed to FER2013 size
...
parser.add_argument('--dataset', default='fer2013', type=str)  # Changed to fer2013
parser.add_argument('--data_path', default='/userHome/userhome1/automl_undergraduate/FER_Models/datasets/FER2013/fer2013_modified.csv', type=str)
...
def main():
    ...
    file_names = np.arange(len(df))  # Create indices for all samples
    ...
    results_df.to_csv('AdaDF_FER2013_test_results.csv', index=False)
