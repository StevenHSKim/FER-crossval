...
parser.add_argument('--num_classes', default=7, type=int)       # ExpW has 7 classes
...
parser.add_argument('--dataset', default='expw', type=str)
parser.add_argument('--data_path', default='/userHome/userhome1/automl_undergraduate/FER_Models/datasets/ExpW_dataset/label/label.lst', type=str)
parser.add_argument('--image_dir', default='/userHome/userhome1/automl_undergraduate/FER_Models/datasets/ExpW_dataset/aligned_image', type=str)
...
def get_expw_dataloaders(data_path, image_dir, batch_size=64, num_workers=2, 
    ...
    full_dataset = ExpWDataset(data_path, image_dir, transform=data_transforms)

    # Create subsets using indices
    train_dataset = torch.utils.data.Subset(full_dataset, train_indices)
    
    # Create validation and test datasets with validation transforms
    full_dataset_val = ExpWDataset(data_path, image_dir, transform=data_transforms_val)
...
def train(train_loader, model, device, criterion, criterion_kld, optimizer, LD, epoch):
...
def validate(loader, model, device, criterion, epoch, phase='train'):
    losses = AverageMeter()
    accs = AverageMeter()
    all_targets = []
    all_preds = []
...
def main():
    ...
    # Dataset loading
    dataset = ExpWDataset(args.data_path, args.image_dir)
    ...
    for iteration, (train_val_indices, test_indices) in enumerate(splits):
        ...
        train_loader, val_loader, test_loader = get_expw_dataloaders(
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            train_loss, train_loss_ce, train_loss_kld, alpha_1, alpha_2, train_acc, outputs_new, targets_new, weights_new = train(
                train_loader, model, device, criterion, criterion_kld, optimizer, LD, epoch)
            ...
            val_loss, val_acc, val_balanced_acc = validate(val_loader, model, device, criterion, epoch, phase='validation')
            test_loss, test_acc, test_balanced_acc = validate(test_loader, model, device, criterion, epoch, phase='test')

            if test_acc > best_acc:
                # Create  directory if it doesn't exist
                checkpoint_path = '/userHome/userhome1/automl_undergraduate/FER_Models/FER_Models/AdaDF/checkpoints/expw'
                ...
                for filename in os.listdir(checkpoint_path):
                    if filename.startswith(f"expw_iter{iteration+1}_epoch") and filename.endswith(".pth"):
                        previous_checkpoint = os.path.join(checkpoint_path, filename)
                        break
                ...
                # Save new checkpoint
                best_checkpoint_path = os.path.join(checkpoint_path, f"expw_iter{iteration+1}_epoch{epoch}_acc{test_acc:.4f}_bacc{test_balanced_acc:.4f}.pth")
                torch.save({
                    'epoch': epoch,
    ...
    results_df.to_csv('AdaDF_ExpW_test_results.csv', index=False)
