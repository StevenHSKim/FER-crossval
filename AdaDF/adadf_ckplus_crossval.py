...
parser.add_argument('--num_classes', default=7, type=int)       # CKPlus has 7 classes
...
parser.add_argument('--dataset', default='ckplus', type=str)
parser.add_argument('--data_path', default='/userHome/userhome1/automl_undergraduate/FER_Models/datasets/CKPlus/ckplus_labels.csv', type=str)
parser.add_argument('--image_dir', default='/userHome/userhome1/automl_undergraduate/FER_Models/datasets/CKPlus/ckplus_images', type=str)
...
class CKPlusDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Read as grayscale image and convert to RGB
...
def get_ckplus_dataloaders(data_path, image_dir, batch_size=64, num_workers=2, 
    ...
    full_dataset = CKPlusDataset(data_path, image_dir, transform=data_transforms)

    # Create subsets using indices
    train_dataset = torch.utils.data.Subset(full_dataset, train_indices)
    
    # Create validation and test datasets with validation transforms
    full_dataset_val = CKPlusDataset(data_path, image_dir, transform=data_transforms_val)
...
def save_checkpoint(state, iteration, val_acc, balanced_acc, epoch, checkpoint_dir='/userHome/userhome1/automl_undergraduate/FER_Models/FER_Models/Ada-DF/checkpoints'):
   ...
   checkpoint_path = os.path.join(
       checkpoint_dir,
       f"ckplus_iter{iteration+1}_epoch{epoch}_acc{val_acc:.4f}_bacc{balanced_acc:.4f}.pth"
   )
...
def main():
    ...
    for iteration, (train_val_indices, test_indices) in enumerate(splits):
        ...
        train_loader, val_loader, test_loader = get_ckplus_dataloaders(
    ...
    results_df.to_csv('AdaDF_CKPlus_test_results.csv', index=False)
