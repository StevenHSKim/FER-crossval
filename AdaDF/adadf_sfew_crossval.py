...
parser.add_argument('--num_classes', default=7, type=int)       # SFEW2.0 has 7 classes
# Method-specific configurations (keeping same as FERPlus)
...
parser.add_argument('--dataset', default='sfew', type=str)
parser.add_argument('--data_path', default='/userHome/userhome1/automl_undergraduate/FER_Models/datasets/SFEW2.0/sfew_2.0_labels.csv', type=str)
parser.add_argument('--image_dir', default='/userHome/userhome1/automl_undergraduate/FER_Models/datasets/SFEW2.0/sfew2.0_images', type=str)
...
# SFEW2.0 Dataset Class
class SFEW2Dataset(Dataset):
...
def get_sfew_dataloaders(data_path, image_dir, batch_size=64, num_workers=2, 
                         train_indices=None, val_indices=None, test_indices=None):
    mean = (0.485, 0.456, 0.406)
    std = (0.229, 0.224, 0.225)

    data_transforms = transforms.Compose([
        transforms.Resize((112, 112)),  # Resize SFEW2.0 images (143x181) to match model input
    ...
    full_dataset = SFEW2Dataset(data_path, image_dir, transform=data_transforms)

    # Create subsets using indices
    train_dataset = torch.utils.data.Subset(full_dataset, train_indices)
    
    # Create validation and test datasets with validation transforms
    full_dataset_val = SFEW2Dataset(data_path, image_dir, transform=data_transforms_val)
...
def save_checkpoint(state, iteration, val_acc, balanced_acc, epoch, checkpoint_dir='/userHome/userhome1/automl_undergraduate/FER_Models/FER_Models/Ada-DF/checkpoints/sfew2.0'):
    if not os.path.exists(checkpoint_dir):
        os.makedirs(checkpoint_dir)
        
    # Remove previous checkpoint for this iteration if exists
    previous_checkpoint = None
    for filename in os.listdir(checkpoint_dir):
        if filename.startswith(f"sfew_iter{iteration+1}_epoch") and filename.endswith(".pth"):
    ...
    checkpoint_path = os.path.join(
        checkpoint_dir, 
        f"sfew_iter{iteration+1}_epoch{epoch}_acc{val_acc:.4f}_bacc{balanced_acc:.4f}.pth"
    )
...
def main():
    ...
    for iteration, (train_val_indices, test_indices) in enumerate(splits):
        ...
        train_loader, val_loader, test_loader = get_sfew_dataloaders(
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if val_loss < best_loss:
                ...
                }, iteration, val_acc, val_balanced_acc, epoch)
            else:
                patience_counter += 1
    ...
    results_df.to_csv('AdaDF_SFEW_test_results.csv', index=False)
