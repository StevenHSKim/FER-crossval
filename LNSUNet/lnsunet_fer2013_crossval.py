...
def get_args():
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='/userHome/userhome1/automl_undergraduate/FER_Models/FER_Models/LNSU-Net/checkpoints/fer2013', help='Checkpoint directory')
...
class FER2013Dataset(torch.utils.data.Dataset):
    def __init__(self, data_path, transform=None, img_size=112, indices=None):
    ...
    def __getitem__(self, idx):
        ...
        return img, torch.tensor(label), img1

    def __len__(self):
        return len(self.data)
        
    def get_labels(self):
        return self.data['emotion'].values
    ...
    for epoch in tqdm(range(start_epoch, args.epochs)):
        ...
        if val_acc > best_val_acc:
            ...
            for filename in os.listdir(args.checkpoint_dir):
                if filename.startswith(f"fer2013_iter{iteration+1}_epoch") and filename.endswith(".pth"):
            ...
            best_checkpoint_path = os.path.join(
                args.checkpoint_dir, 
                f"fer2013_iter{iteration+1}_epoch{epoch}_acc{val_acc:.4f}_bacc{val_balanced_acc:.4f}.pth"
            )
...
def run_train_test():
    ...
    for iteration in range(args.iterations):
        ...
        train_loader = DataLoader(
            FER2013Dataset(data_path, indices=train_indices),
        ...
        val_loader = DataLoader(
            FER2013Dataset(data_path, indices=val_indices),
            batch_size=args.batch_size,
            shuffle=False,
            num_workers=args.num_workers,
            pin_memory=True,
        ...
        test_loader = DataLoader(
            FER2013Dataset(data_path, indices=test_indices),
            batch_size=args.batch_size,
            shuffle=False,
            num_workers=args.num_workers,
            pin_memory=True,
    ...
    results_df.to_csv(os.path.join(args.checkpoint_dir, "LNSUNet_FER2013_test_results.csv"), index=False)
