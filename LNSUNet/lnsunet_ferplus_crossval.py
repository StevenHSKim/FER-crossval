...
def get_args():
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='/userHome/userhome1/automl_undergraduate/FER_Models/FER_Models/LNSU-Net/checkpoints/ferplus', help='Checkpoint directory')
...
class FERPlusDataset(torch.utils.data.Dataset):
    def __init__(self, data_path, label_path, transform=None, img_size=112, indices=None):
        ...
        for _, row in df.iterrows():
            image_path = os.path.join(self.data_path, row['Image name'])
    ...
    def __getitem__(self, idx):
        ...
        img = Image.open(img_path)  # Already grayscale
...
class LSR2(nn.Module):
    ...
    def _smooth_label(self, target, length, smooth_factor):
        ...
        mask = (one_hot==0)
        balance_weight = torch.tensor([0.95124031, 4.36690391, 1.71143654, 0.25714585, 0.6191221, 1.74056738, 0.48617274, 0.5]).to(one_hot.device)  # Added weight for 8th class
...
def train(args, model, train_loader, val_loader, current_lr, iteration):
    ...
    for epoch in tqdm(range(start_epoch, args.epochs)):
        ...
        for idx, (expression_img, expression_label, expression_img1) in enumerate(train_loader):
            ...
            balance_weight = torch.tensor([0.95124031, 4.36690391, 1.71143654, 0.25714585, 0.6191221, 1.74056738, 0.48617274, 0.5]).cuda().view(8,1)  # Added weight for 8th class
        ...
        if val_acc > best_val_acc:
            ...
            for filename in os.listdir(args.checkpoint_dir):
                if filename.startswith(f"ferplus_iter{iteration+1}_epoch") and filename.endswith(".pth"):
            ...
            best_checkpoint_path = os.path.join(
                args.checkpoint_dir, 
                f"ferplus_iter{iteration+1}_epoch{epoch}_acc{val_acc:.4f}_bacc{val_balanced_acc:.4f}.pth"
            )
...
def run_train_test():
    ...
    # Image and label paths for FERPlus
    image_path = "/userHome/userhome1/automl_undergraduate/FER_Models/datasets/FERPlus/FERPlus_Image"
    label_path = "/userHome/userhome1/automl_undergraduate/FER_Models/datasets/FERPlus/FERPlus_Label_modified.csv"
    
    # 전체 데이터셋 인덱스 생성
    df = pd.read_csv(label_path)
    total_indices = range(len(df))
    ...
    for iteration in range(args.iterations):
        ...
        model = SwinTransFER(swin=swin, swin_num_features=768, num_classes=8, cam=True)

        train_loader = DataLoader(
            FERPlusDataset(image_path, label_path, indices=train_indices),
        ...
        val_loader = DataLoader(
            FERPlusDataset(image_path, label_path, indices=val_indices),
            batch_size=args.batch_size,
            shuffle=False,
            num_workers=args.num_workers,
            pin_memory=True,
        ...
        test_loader = DataLoader(
            FERPlusDataset(image_path, label_path, indices=test_indices),
            batch_size=args.batch_size,
            shuffle=False,
            num_workers=args.num_workers,
            pin_memory=True,
    ...
    results_df.to_csv(os.path.join(args.checkpoint_dir, "LNSUNet_FERPlus_test_results.csv"), index=False)
