...
def get_args():
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='/userHome/userhome1/automl_undergraduate/FER_Models/FER_Models/LNSU-Net/checkpoints/rafdb', help='Checkpoint directory')
...
class RAFDBDataset(torch.utils.data.Dataset):
    def __init__(self, data_path, label_path, transform=None, img_size=112, indices=None):
        ...
        # 최적화된 transform 파이프라인
        if transform:
            self.transform = transform
        else:
            self.transform = transforms.Compose([
                transforms.Resize([img_size, img_size]),
                transforms.ToTensor(),  # numpy 변환 없이 바로 tensor로
        ...
        for line in data:
            line = line.strip('\n').split(" ")
            image_name = line[0].split(".")[0] + '_aligned.jpg'
            image_path = os.path.join(self.data_path, image_name)
            all_paths.append(image_path)
            all_labels.append(int(line[1]) - 1)
    ...
    def __getitem__(self, idx):
        ...
        img = self.transform(img)  # 이미 tensor로 변환됨
        label = torch.tensor(self.labels[idx])
        img1 = transforms.RandomHorizontalFlip(p=1.0)(img)  # tensor에 직접 적용
...
def train(args, model, train_loader, val_loader, current_lr, iteration):
    ...
    for epoch in tqdm(range(start_epoch, args.epochs)):
        ...
        # Save checkpoint when validation acc improves
        if val_acc > best_val_acc:
            ...
            for filename in os.listdir(args.checkpoint_dir):
                if filename.startswith(f"rafdb_iter{iteration+1}_epoch") and filename.endswith(".pth"):
            ...
            best_checkpoint_path = os.path.join(
                args.checkpoint_dir, 
                f"rafdb_iter{iteration+1}_epoch{epoch}_acc{val_acc:.4f}_bacc{val_balanced_acc:.4f}.pth"
            )
...
def run_train_test():
    ...
    image_path = "/userHome/userhome1/automl_undergraduate/FER_Models/datasets/raf-basic/Image/aligned"
    label_path = "/userHome/userhome1/automl_undergraduate/FER_Models/datasets/raf-basic/EmoLabel/list_patition_label.txt"
    
    # 전체 데이터셋 인덱스 생성
    df = pd.read_csv(label_path, sep=' ', header=None, names=['name', 'label'])
    ...
    for iteration in range(args.iterations):
        ...
        train_loader = DataLoader(
            RAFDBDataset(image_path, label_path, indices=train_indices),
            batch_size=args.batch_size,
            shuffle=True,
            num_workers=args.num_workers,
            pin_memory=True,
            prefetch_factor=2,  # 데이터 미리 로딩
            persistent_workers=True  # 워커 재사용
        )
        
        val_loader = DataLoader(
            RAFDBDataset(image_path, label_path, indices=val_indices),
            batch_size=args.batch_size,
            shuffle=False,
            num_workers=args.num_workers,
            pin_memory=True,
        ...
        test_loader = DataLoader(
            RAFDBDataset(image_path, label_path, indices=test_indices),
            batch_size=args.batch_size,
            shuffle=False,
            num_workers=args.num_workers,
            pin_memory=True,
    ...
    results_df.to_csv(os.path.join(args.checkpoint_dir, "LNSUNet_RAFDB_test_results.csv"), index=False)
