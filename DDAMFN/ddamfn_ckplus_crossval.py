...
def train(train_loader, val_loader, device, args, iteration):
    model = DDAMNet(num_class=7, num_head=args.num_head)  # 7 classes for CKPlus
...
def run_train_test():
    ...
    for iteration, (train_val_indices, test_indices) in enumerate(splits):
        ...
        val_dataset = CKPlusDataSet(args.ckplus_path, phase='val', indices=val_indices, transform=data_transforms_val)
        ...
        test_dataset = CKPlusDataSet(args.ckplus_path, phase='test', indices=test_indices, transform=data_transforms_val)
    ...
    results_df.to_csv('DDAMFN_CKPlus_test_results.csv', index=False)
