...
def parse_args():
    ...
    parser.add_argument('--val_size', type=float, default=0.25, help='Fraction of remaining data (after test split) to use for validation')
...
def train(train_loader, val_loader, device, args, iteration):
    ...
    for epoch in tqdm(range(1, args.epochs + 1)):
        ...
        with torch.no_grad():
            ...
            # Save checkpoint if accuracy is higher than threshold and it's the best so far
            if acc > 0.65 and acc == best_acc:
                checkpoint_path = os.path.join('/userHome/userhome1/automl_undergraduate/FER_Models/FER_Models/DDAMFN/checkpoints', 
                                             f"fer2013_iter{iteration+1}_epoch{epoch}_acc{acc:.4f}_bacc{balanced_acc:.4f}.pth")
                torch.save({
                    'iter': epoch, 
...
def run_train_test():
    ...
    # Lists to store results for statistical analysis
    all_accuracies = []
    all_val_losses = []
    all_test_losses = []
    ...
    splits = list(ss.split(data_indices, labels))
    ...
    for iteration, (train_val_indices, test_indices) in enumerate(splits):
        ...
        # Adjust validation split ratio to achieve 6:2:2 split
        val_ratio = 0.25  # 0.25 of 80% = 20% of total
        train_indices, val_indices = train_test_split(train_val_indices, test_size=val_ratio, random_state=iteration)
        
        # Print dataset sizes
        print(f'Train set size: {len(train_indices)} ({len(train_indices)/len(data_indices)*100:.1f}%)')
        print(f'Validation set size: {len(val_indices)} ({len(val_indices)/len(data_indices)*100:.1f}%)')
        print(f'Test set size: {len(test_indices)} ({len(test_indices)/len(data_indices)*100:.1f}%)')
        ...
        val_dataset = FER2013Dataset(args.fer_path, indices=val_indices, transform=data_transforms_val)
        ...
        test_dataset = FER2013Dataset(args.fer_path, indices=test_indices, transform=data_transforms_val)
        ...
        best_checkpoint_path, best_acc, val_loss = train(train_loader, val_loader, device, args, iteration)
        if best_checkpoint_path:
            ...
            results.append([iteration + 1, test_acc, test_balanced_acc, val_loss, test_running_loss])
            print(f"Test Accuracy: {test_acc}, Test Balanced Accuracy: {test_balanced_acc}, Val Loss: {val_loss}, Test Loss: {test_running_loss}")

    # Save detailed results
    results_df = pd.DataFrame(results, columns=['Iteration', 'Test Accuracy', 'Balanced Accuracy', 'Val Loss', 'Test Loss'])
    results_df.to_csv('DDAMFN_FER2013_test_results.csv', index=False)
    ...
    mean_balanced_acc = np.mean(all_balanced_accuracies)
    std_balanced_acc = np.std(all_balanced_accuracies)
    ...
    # Get total parameters for logging
    model = DDAMNet(num_class=7, num_head=args.num_head)
    parameters = sum(p.numel() for p in model.parameters() if p.requires_grad) / 1_000_000

    # Prepare the new result row for total_results.csv
    # Format strings with mean and standard deviation
    acc_str = f"{mean_accuracy:.4f}±{std_accuracy:.4f}"
    bacc_str = f"{mean_balanced_acc:.4f}±{std_balanced_acc:.4f}"
    val_loss_str = f"{mean_val_loss:.4f}±{std_val_loss:.4f}"
    test_loss_str = f"{mean_test_loss:.4f}±{std_test_loss:.4f}"
