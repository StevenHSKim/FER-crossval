...
class FERPlusDataSet(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = Image.open(path).convert('RGB')  # Convert grayscale to RGB
...
def train(train_loader, val_loader, device, args, iteration):
    model = DDAMNet(num_class=8, num_head=args.num_head)  # 8 classes for FERPlus
    ...
    for epoch in tqdm(range(1, args.epochs + 1)):
        ...
        with torch.no_grad():
            ...
            if acc > 0.8 and acc == best_acc:
                checkpoint_path = os.path.join('/userHome/userhome1/automl_undergraduate/FER_Models/FER_Models/DDAMFN/checkpoints', f"ferplus_iter{iteration+1}_epoch{epoch}_acc{acc:.4f}_bacc{balanced_acc:.4f}.pth")
...
def run_train_test():
    ...
    for iteration, (train_val_indices, test_indices) in enumerate(splits):
        ...
        val_dataset = FERPlusDataSet(args.ferplus_path, phase='val', indices=val_indices, transform=data_transforms_val)
        ...
        test_dataset = FERPlusDataSet(args.ferplus_path, phase='test', indices=test_indices, transform=data_transforms_val)
    ...
    results_df.to_csv('DDAMFN_FERPlus_test_results.csv', index=False)
    ...
    mean_test_loss = np.mean([result[3] for result in results])  # Get test loss from results
    std_test_loss = np.std([result[3] for result in results])
    mean_balanced_acc = np.mean([result[2] for result in results])  # Get balanced accuracy from results
    ...
    new_result = {
        ...
        'Val Loss': f"{mean_test_loss:.4f}±{std_test_loss:.4f}",  # Using test loss as val loss is not tracked
