...
def parse_args():
    ...
    parser.add_argument('--ferplus_path', type=str, default='/userHome/userhome1/automl_undergraduate/FER_Models/datasets/FERPlus/', help='FERPlus dataset path.')
...
class FERPlusDataSet(data.Dataset):
    def __init__(self, ferplus_path, phase, indices, transform=None):
        self.phase = phase
        self.transform = transform
        self.ferplus_path = ferplus_path

        # header=0으로 지정하여 첫 행을 헤더로 인식
        df = pd.read_csv(os.path.join(self.ferplus_path, 'FERPlus_Label_modified.csv'),
                        header=0,  # 첫 행을 헤더로 사용
                        dtype={'Image name': str, 'label': int})
        
        # 인덱스 범위 확인 및 적용
        indices = np.array(indices)
        indices = indices[indices < len(df)]  # 유효한 인덱스만 선택
        
        self.file_names = df['Image name'].values[indices]
        self.labels = df['label'].values[indices]

        self.file_paths = [os.path.join(self.ferplus_path, 'FERPlus_Image', f) for f in self.file_names]
    ...
    for epoch in tqdm(range(1, epochs + 1)):
        ...
        with torch.no_grad():
            ...
            if acc > 0.8 and acc == best_acc:
                best_checkpoint_path = os.path.join('/userHome/userhome1/automl_undergraduate/FER_Models/FER_Models/DAN/checkpoints', f"ferplus_iter{iteration+1}_epoch{epoch}_acc{acc}_bacc{balanced_acc}.pth")
                torch.save({
                    'epoch': epoch,
...
def run_train_test():
    ...
    # 데이터 로드 및 레이블 처리
    df = pd.read_csv(os.path.join(args.ferplus_path, 'FERPlus_Label_modified.csv'), 
                    header=0,
                    dtype={'Image name': str, 'label': int})
    ...
    for iteration, (train_val_indices, test_indices) in enumerate(splits):
        ...
        train_dataset = FERPlusDataSet(args.ferplus_path, phase='train', indices=train_indices, transform=data_transforms)
        ...
        val_dataset = FERPlusDataSet(args.ferplus_path, phase='validation', indices=val_indices, transform=val_transforms)
        ...
        test_dataset = FERPlusDataSet(args.ferplus_path, phase='test', indices=test_indices, transform=val_transforms)
        ...
        criterion_af = AffinityLoss(device, num_class=num_classes)  # num_classes 전달
    ...
    results_df.to_csv('DAN_FERPlus_test_results.csv', index=False)
