...
class SFEW2Dataset(data.Dataset):
    def __init__(self, sfew_path, phase, indices, transform=None):
        ...
        label_path = os.path.join(self.sfew_path, 'sfew_2.0_labels.csv')
        df = pd.read_csv(label_path)
        
        # Select only image_name and label columns
        image_names = df['image_name'].values
        labels = df['label'].values
        
        # Filter by valid indices
        indices = np.array(indices)
        indices = indices[indices < len(image_names)]
...
def train(train_loader, val_loader, model, criterion_cls, criterion_af, criterion_pt, optimizer, scheduler, device, epochs, patience, iteration):
    ...
    for epoch in tqdm(range(1, epochs + 1)):
        ...
        with torch.no_grad():
            ...
            # Save checkpoint when best_acc is updated
            if val_loss < best_loss:
                best_loss = val_loss
                checkpoint_dir = '/userHome/userhome1/automl_undergraduate/FER_Models/FER_Models/DAN/checkpoint/sfew2.0'
                ...
                best_checkpoint_path = os.path.join(checkpoint_dir, f"sfew2.0_iter{iteration+1}_epoch{epoch}_loss{val_loss:.4f}_bacc{balanced_acc}.pth")
                torch.save({
                    'epoch': epoch,
...
def run_train_test():
    ...
    for iteration, (train_val_indices, test_indices) in enumerate(splits):
        ...
        val_dataset = SFEW2Dataset(args.sfew_path, phase='validation', indices=val_indices, transform=val_transforms)
        ...
        test_dataset = SFEW2Dataset(args.sfew_path, phase='test', indices=test_indices, transform=val_transforms)
    ...
    results_df.to_csv('DAN_SFEW_test_results.csv', index=False)
