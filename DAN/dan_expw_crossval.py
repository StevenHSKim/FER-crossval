...
class ExpWDataSet(data.Dataset):
    def __init__(self, expw_path, phase, indices, transform=None):
        self.phase = phase
        self.transform = transform
        self.expw_path = expw_path
        
        # 레이블 파일 읽기
        label_path = os.path.join(self.expw_path, 'label/label.lst')
        image_names = []
        labels = []
        ...
        with open(label_path, 'r') as f:
            for line in f:
                ...
                image_name = parts[0]
                label = int(parts[-1])  # 마지막 값이 expression label
                
                # 이미지 파일이 실제로 존재하는지 확인
                image_path = os.path.join(self.expw_path, 'aligned_image', image_name)
        ...
        indices = indices[indices < len(image_names)]  # 유효한 인덱스만 선택
        
        self.file_names = np.array(image_names)[indices]
        self.labels = np.array(labels)[indices]
        
        self.file_paths = [os.path.join(self.expw_path, 'aligned_image', f) for f in self.file_names]
    ...
    for epoch in tqdm(range(1, epochs + 1)):
        ...
        with torch.no_grad():
            ...
            if acc > best_acc:
                ...
                checkpoint_dir = '/userHome/userhome1/automl_undergraduate/FER_Models/FER_Models/DAN/checkpoints/expw'
                ...
                # 이전 checkpoint가 있다면 삭제
                if previous_checkpoint and os.path.exists(previous_checkpoint):
                    os.remove(previous_checkpoint)
                
                # 새로운 checkpoint 저장
                best_checkpoint_path = os.path.join(checkpoint_dir, f"expw_iter{iteration+1}_epoch{epoch}_acc{acc}_bacc{balanced_acc}.pth")
                torch.save({
                    'epoch': epoch,
...
def run_train_test():
    ...
    with open(os.path.join(args.expw_path, 'label/label.lst'), 'r') as f:
        for line in f:
            parts = line.strip().split()
            image_name = parts[0]
            label = int(parts[-1])
            ...
            # 이미지 파일이 실제로 존재하는지 확인
            image_path = os.path.join(args.expw_path, 'aligned_image', image_name)
            if os.path.exists(image_path):
                image_names.append(image_name)
                labels.append(label)
    ...
    for iteration, (train_val_indices, test_indices) in enumerate(splits):
        ...
        train_dataset = ExpWDataSet(args.expw_path, phase='train', indices=train_indices, transform=data_transforms)
        ...
        val_dataset = ExpWDataSet(args.expw_path, phase='validation', indices=val_indices, transform=val_transforms)
        ...
        test_dataset = ExpWDataSet(args.expw_path, phase='test', indices=test_indices, transform=val_transforms)
    ...
    results_df.to_csv('DAN_ExpW_test_results.csv', index=False)
    ...
    # validation loss와 test loss 계산
    all_val_losses = []
    all_test_losses = []
    
    for result in results:
        all_test_losses.append(result[3])  # Loss column from results
    ...
    new_result = {
        ...
        'Val Loss': f"{mean_test_loss:.4f}±{std_test_loss:.4f}",  # Using test loss as validation loss
