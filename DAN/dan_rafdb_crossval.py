...
from networks.dan import DAN  # DAN 모델을 임포트
...
def parse_args():
    ...
    parser.add_argument('--raf_path', type=str, default='/userHome/userhome1/automl_undergraduate/FER_Models/DAN/datasets/raf-basic/', help='Raf-DB dataset path.')
    ...
    for epoch in tqdm(range(1, epochs + 1)):
        ...
        with torch.no_grad():
            ...
            if acc > 0.85 and acc == best_acc:
                best_checkpoint_path = os.path.join('/userHome/userhome1/automl_undergraduate/FER_Models/DAN/checkpoints', f"iter{iteration+1}_epoch{epoch}_acc{acc}_bacc{balanced_acc}.pth")
                torch.save({
                    'epoch': epoch,
...
def run_train_test():
    ...
    results_df.to_csv('DAN_test_results.csv', index=False)
