...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='expw', help='dataset')
...
def get_datasets(args, train_val_indices, test_indices, random_state):
    datapath = '/userHome/userhome1/automl_undergraduate/FER_Models/datasets/ExpW_dataset'
    label_file = os.path.join(datapath, 'label/label.lst')
    ...
    for line in lines:
        ...
        img_name = parts[0]
        label = int(parts[-1])  # Last element is the expression label
        
        # Check if aligned image exists
        img_path = os.path.join(datapath, 'aligned_image', img_name)
        if os.path.exists(img_path):
            file_names.append(img_name)
            labels.append(label)
    
    file_paths = [os.path.join(datapath, 'aligned_image', f) for f in file_names]
    labels = np.array(labels)  # 0:Angry, 1:Disgust, 2:Fear, 3:Happy, 4:Sad, 5:Surprise, 6:Neutral

    train_indices, val_indices = train_test_split(train_val_indices, test_size=args.val_size, random_state=random_state)

    train_dataset = ExpWDataSet(np.array(file_paths)[train_indices], np.array(labels)[train_indices], transform=get_data_transforms(train=True), basic_aug=True)
    val_dataset = ExpWDataSet(np.array(file_paths)[val_indices], np.array(labels)[val_indices], transform=get_data_transforms(train=False))
    test_dataset = ExpWDataSet(np.array(file_paths)[test_indices], np.array(labels)[test_indices], transform=get_data_transforms(train=False))
...
def train(train_loader, val_loader, model, criterion, optimizer, scheduler, device, epochs, patience, iteration):
    best_acc = 0
    best_loss = float('inf')
    best_val_loss = float('inf')  # 추가: 최종적으로 반환할 best validation loss
    patience_counter = 0
    save_path = None
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_loss < best_loss:
            best_loss = val_loss
            best_val_loss = val_loss  # 추가: 최상의 validation loss 업데이트
            patience_counter = 0
        else:
            patience_counter += 1
        ...
        if val_acc > 0.6 and val_acc > best_acc:
            best_acc = val_acc
            print(f"Best accuracy: {best_acc:.4f}")
            
            save_path = os.path.join('/userHome/userhome1/automl_undergraduate/FER_Models/FER_Models/POSTER/checkpoint', f"expw_iter{iteration+1}_epoch{epoch}_acc{val_acc:.4f}.pth")
            torch.save({'epoch': epoch, 'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict()}, save_path)
            print('Model saved.')

    return model, best_acc, save_path, best_val_loss
...
def run_train_test():
    ...
    all_accuracies = []
    best_accuracies = []
    all_val_losses = []  # 추가: validation loss 저장
    all_test_losses = [] # 추가: test loss 저장
    results = []
    ...
    for iteration, (train_val_indices, test_indices) in enumerate(splits):
        ...
        model, best_acc, best_checkpoint_path, best_val_loss = train(train_loader, val_loader, model, criterion, optimizer, scheduler, device, args.epochs, args.patience, iteration)
        ...
        if args.plot_cm:
            ...
            plot_confusion_matrix(cm, labels_name, f'ExpW Iteration {iteration + 1}', test_acc)

    # 기존의 결과 저장
    results_df = pd.DataFrame(results, columns=['Iteration', 'Test Accuracy', 'Balanced Accuracy', 'Validation Loss', 'Test Loss'])
    results_df.to_csv('POSTER_ExpW_test_results.csv', index=False)
