...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='ckplus', help='dataset')
...
def get_datasets(args, train_val_indices, test_indices, random_state):
    datapath = '/userHome/userhome1/automl_undergraduate/FER_Models/datasets/CKPlus'
    df = pd.read_csv(os.path.join(datapath, 'ckplus_labels.csv'))

    file_names = df['image_name'].values
    labels = df['label'].values
    file_paths = [os.path.join(datapath, 'ckplus_images', name) for name in file_names]

    train_indices, val_indices = train_test_split(train_val_indices, test_size=args.val_size, random_state=random_state)

    train_dataset = CKPlusDataSet(np.array(file_paths)[train_indices], np.array(labels)[train_indices], transform=get_data_transforms(train=True), basic_aug=True)
    val_dataset = CKPlusDataSet(np.array(file_paths)[val_indices], np.array(labels)[val_indices], transform=get_data_transforms(train=False))
    test_dataset = CKPlusDataSet(np.array(file_paths)[test_indices], np.array(labels)[test_indices], transform=get_data_transforms(train=False))

    num_classes = 7  # CKPlus has 7 emotion classes
...
def train(train_loader, val_loader, model, criterion, optimizer, scheduler, device, epochs, patience, iteration):
    ...
    best_loss = float('inf')
    patience_counter = 0
    checkpoint_dir = '/userHome/userhome1/automl_undergraduate/FER_Models/FER_Models/POSTER/checkpoint/ckplus'
    dataset_name = 'ckplus'
    ...
    return model, best_acc, best_checkpoint_path, final_val_loss
...
def run_train_test():
    ...
    for iteration, (train_val_indices, test_indices) in enumerate(splits):
        ...
        if args.plot_cm:
            labels_name = ['AN', 'CO', 'DI', 'FE', 'HA', 'SA', 'SU']
            plot_confusion_matrix(cm, labels_name, f'CKPlus Iteration {iteration + 1}', test_acc)
    ...
    results_df.to_csv('POSTER_CKPlus_test_results.csv', index=False)
