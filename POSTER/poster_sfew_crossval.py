...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='sfew', help='dataset')
    ...
    parser.add_argument('--t_max', default=100, type=int, help='T_max for CosineAnnealingLR scheduler')
...
class SFEW2Dataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = cv2.imread(path)  # 컬러로 읽기
        sample = cv2.cvtColor(sample, cv2.COLOR_BGR2RGB)  # BGR을 RGB로 변환
...
def get_datasets(args, train_val_indices, test_indices, random_state):
    datapath = '/userHome/userhome1/automl_undergraduate/FER_Models/datasets/SFEW2.0'
    df = pd.read_csv(os.path.join(datapath, 'sfew_2.0_labels.csv'))

    file_names = df['image_name'].values
    labels = df['label'].values  # 0:'Angry', 1:'Disgust', 2:'Fear', 3:'Happy', 4:'Neutral', 5:'Sad', 6:'Surprise'
    file_paths = [os.path.join(datapath, 'sfew2.0_images', name) for name in file_names]

    train_indices, val_indices = train_test_split(train_val_indices, test_size=args.val_size, random_state=random_state)

    train_dataset = SFEW2Dataset(np.array(file_paths)[train_indices], np.array(labels)[train_indices], transform=get_data_transforms(train=True), basic_aug=True)
    val_dataset = SFEW2Dataset(np.array(file_paths)[val_indices], np.array(labels)[val_indices], transform=get_data_transforms(train=False))
    test_dataset = SFEW2Dataset(np.array(file_paths)[test_indices], np.array(labels)[test_indices], transform=get_data_transforms(train=False))

    num_classes = 7  # SFEW2.0 has 7 emotion classes
...
def train(train_loader, val_loader, model, criterion, optimizer, scheduler, device, epochs, patience, iteration):
    ...
    patience_counter = 0
    save_path = None
    final_val_loss = float('inf')  # 최종 validation loss 저장용
    ...
    for epoch in range(1, epochs + 1):
        ...
        final_val_loss = val_loss  # 현재 validation loss 저장
        ...
        if val_loss < best_loss:
            best_acc = val_acc
            checkpoint_dir = '/userHome/userhome1/automl_undergraduate/FER_Models/FER_Models/POSTER/checkpoint/sfew2.0'
            ...
            best_checkpoint_path = os.path.join(
                checkpoint_dir,
                f"sfew2.0_iter{iteration+1}_epoch{epoch}_acc{val_acc:.4f}.pth"
            )
    ...
    return model, best_acc, best_checkpoint_path, final_val_loss  # save_path 대신 best_checkpoint_path 반환
...
def run_train_test():
    ...
    all_accuracies = []
    best_accuracies = []
    all_val_losses = []  # validation loss 추적을 위한 리스트 추가
    all_test_losses = []  # test loss 추적을 위한 리스트 추가
    results = []
    ...
    for iteration, (train_val_indices, test_indices) in enumerate(splits):
        ...
        model = pyramid_trans_expr(img_size=112, num_classes=num_classes, type=args.modeltype)
        ...
        model, best_acc, best_checkpoint_path, final_val_loss = train(train_loader, val_loader, model, criterion, optimizer, scheduler, device, args.epochs, args.patience, iteration)
        test_acc, test_balanced_acc, test_loss, cm, pre_labels, gt_labels = test(test_loader, model, best_checkpoint_path, device, criterion)

        all_accuracies.append(test_acc)
        best_accuracies.append(best_acc)
        all_val_losses.append(final_val_loss)  # validation loss 저장
        all_test_losses.append(test_loss)  # test loss 저장
        results.append([iteration + 1, test_acc, test_balanced_acc, final_val_loss, test_loss])
        print(f"Test Accuracy: {test_acc}, Test Balanced Accuracy: {test_balanced_acc}, Val Loss: {final_val_loss:.4f}, Test Loss: {test_loss:.4f}")

        if args.plot_cm:
            labels_name = ['AN', 'DI', 'FE', 'HA', 'NE', 'SA', 'SU']
            plot_confusion_matrix(cm, labels_name, f'SFEW2.0 Iteration {iteration + 1}', test_acc)
    ...
    results_df.loc[len(results_df)] = ['Mean±Std', 
                                      ...
                                      "-",
                                      f"{mean_val_loss:.4f}±{std_val_loss:.4f}",
                                      f"{mean_test_loss:.4f}±{std_test_loss:.4f}"]
    
    results_df.to_csv('POSTER_SFEW2_test_results.csv', index=False)
