...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str, default='/userHome/userhome3/timur/Haesung/datasets/FERPlus')
    parser.add_argument('--num_class', type=int, default=8)  # FERPlus has 8 emotions
    parser.add_argument('--img_size', type=int, default=224)
    
    parser.add_argument('--model_name', type=str, default='GSDNet')
    parser.add_argument('--dataset_name', type=str, default='ferplus')
...
class FERPlusDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = Image.open(img_path).convert('RGB')  # Convert grayscale to RGB for consistency
...
def run_train_test():
    ...
    # Load dataset
    df = pd.read_csv(os.path.join(args.data_path, 'FERPlus_Label_modified.csv'))
    
    file_names = df['Image name'].values
    labels = df['label'].values
    file_paths = [os.path.join(args.data_path, 'FERPlus_Image', fname) for fname in file_names]
