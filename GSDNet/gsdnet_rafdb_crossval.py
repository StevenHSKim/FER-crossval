...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str, default='/userHome/userhome3/timur/Haesung/datasets/raf-basic')
    parser.add_argument('--num_class', type=int, default=7)
    parser.add_argument('--img_size', type=int, default=224)
    
    parser.add_argument('--model_name', type=str, default='GSDNet')
    parser.add_argument('--dataset_name', type=str, default='rafdb')
...
def run_train_test():
    ...
    NAME_COLUMN = 0
    LABEL_COLUMN = 1
    df = pd.read_csv(os.path.join(args.data_path, 'EmoLabel/list_patition_label.txt'), sep=' ', header=None)

    file_names = df.iloc[:, NAME_COLUMN].values
    labels = df.iloc[:, LABEL_COLUMN].values - 1
    file_paths = [os.path.join(args.data_path, 'Image/aligned', f.split(".")[0] + "_aligned.jpg") for f in file_names]
