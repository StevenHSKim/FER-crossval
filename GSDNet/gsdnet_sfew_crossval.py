...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str, default='/userHome/userhome3/timur/Haesung/datasets/SFEW')
    parser.add_argument('--num_class', type=int, default=7)
    parser.add_argument('--img_size', type=int, default=224)
    
    parser.add_argument('--model_name', type=str, default='GSDNet')
    parser.add_argument('--dataset_name', type=str, default='sfew')
...
def run_train_test():
    ...
    # Load dataset
    df = pd.read_csv(os.path.join(args.data_path, 'sfew_labels.csv'))
    
    file_names = df['image_name'].values
    labels = df['label'].values
    file_paths = [os.path.join(args.data_path, 'sfew_images', name) for name in file_names]
