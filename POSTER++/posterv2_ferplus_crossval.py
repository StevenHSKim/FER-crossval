...
from models.PosterV2_8cls import pyramid_trans_expr2  # PosterV2_7cls 대신 PosterV2_8cls 사용



def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--data', type=str, default='/userHome/userhome3/timur/Haesung/datasets/FERPlus')
...
def run_train_test():
    ...
    model = pyramid_trans_expr2(img_size=112, num_classes=8)  # 8 classes for FERPlus
    if len(device_ids) > 1:
    ...
    checkpoint_dir = '/userHome/userhome3/timur/Haesung/POSTER++/checkpoint/ferplus'
    ...
    file_names = df.iloc[:, 0].values  # Image name column
    ...
    for iteration, (train_val_indices, test_indices) in enumerate(splits):
        ...
        model = pyramid_trans_expr2(img_size=img_size, num_classes=8)  # 8 classes for FERPlus
